...
```yaml
llm:
  provider: "litellm"   # litellm | ollama_legacy
  model_map:
...
- 既存 `ollama.*` は当面残し、`provider=ollama_legacy` で後方互換にする。
...
1. `BaseLlmClient` と `create_llm_client` を追加（挙動は現状 Ollama と同じ）。
...
- まずは「設計変更のみ」: Orchestrator の依存逆転 + 設定キー追加。
- 挙動は既存 Ollama と等価のまま。
- その後 LiteLLM 実装を追加し、OpenAI で e2e 動作確認する。
